...
// Local mail server on this same cPanel account (outbound external SMTP is
// blocked by the host; localhost is not).
$SMTP_HOST = 'localhost';
$SMTP_PORT = 587;
$SMTP_USER = 'leads@drravigarg.com';
$SMTP_PASS = 'MwtXE-jD6+n$Hq(J';
$FROM_EMAIL = 'leads@drravigarg.com';
// -------------------------------------------------------------------------
...
function smtp_send($host, $port, $user, $pass, $fromName, $recipients, $subject, $body, $replyTo) {
  if ($pass === '' || strpos($pass, 'PASTE_') === 0) return 'smtp not configured';
